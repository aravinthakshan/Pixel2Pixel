...
parser.add_argument('--use_quality_weights', action='store_true', help='Use quality-based sampling weights')
...
def construct_pixel_bank_from_image(img_tensor, file_name_without_ext, bank_dir):
    ...
    for blk_i in range(num_blk_w):
        for blk_j in range(num_blk_h):
            ...
            img_center = img_center[..., center_offset:center_offset + blk_sz, 
            ...
            topk_distances, sort_indices = torch.topk(
                distance, k=NUM_NEIGHBORS, largest=False, sorted=True, dim=-3
            )
    ...
    # Merge the results from all blocks
    ...
    distances = einops.rearrange(distances, '(w1 w2) k h w -> k (w2 h) (w1 w)', 
                                w1=num_blk_w, w2=num_blk_h)
    ...
    np.save(os.path.join(bank_dir, file_name_without_ext + '_distances'), 
            distances.cpu().numpy())
...
def construct_pixel_bank():
    ...
    for image_file in image_files:
        ...
        print(f"Processed {image_file} in {elapsed:.2f}s. Pixel bank shape: {topk.shape}")
...
def compute_quality_weights(distances, alpha=2.0):
    ...
    # Quality score: favor medium distances
    ...
    for image_file in image_files:
        ...
        scheduler = MultiStepLR(optimizer, 
                               milestones=[int(args.epochs*0.5), 
                                         int(args.epochs*0.67), 
                                         int(args.epochs*0.83)], 
                               gamma=0.5)
        ...
        for epoch in range(args.epochs):
            current_loss = train(model, optimizer, img_bank, quality_weights)
            scheduler.step()
            
            # Check for loss stagnation (sigmoid issue)
            if abs(current_loss - prev_loss) < 1e-7:
                stagnant_count += 1
            else:
                stagnant_count = 0
            
            # If loss is stagnant for too long, likely sigmoid collapsed
            if stagnant_count > 50:
                print(f"\nWarning: Loss stagnation detected at epoch {epoch+1}")
                print("This may indicate sigmoid saturation. Consider adjusting learning rate or network architecture.")
            ...
            if (epoch + 1) % 200 == 0:
                ...
                print(f"Epoch {epoch+1}/{args.epochs} - PSNR: {current_psnr:.2f} dB, Loss: {current_loss:.6f}")
        ...
        out_img_save_path = os.path.join(args.out_image, 
                                         os.path.splitext(image_file)[0] + '.png')
        out_img_pil.save(out_img_save_path)
        
        noisy_img_pil = to_pil_image(noisy_img.squeeze(0))
        noisy_img_save_path = os.path.join(args.out_image, 
                                           os.path.splitext(image_file)[0] + '_noisy.png')
        ...
        SSIM, _ = compare_ssim(clean_img_np, out_img_loaded, full=True, 
                              channel_axis=2, win_size=win_size)
        
        print(f"Final Results - PSNR: {PSNR:.2f} dB | SSIM: {SSIM:.4f}")
...
if __name__ == "__main__":
    print("="*60)
    print("Pixel2Pixel Image Denoising")
    print("="*60)
    ...
    print("\nConstructing pixel banks from noisy images...")
